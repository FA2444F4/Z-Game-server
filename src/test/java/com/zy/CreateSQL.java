...
//                int random = (new Random().nextInt(5))+1;
    ...
    public void deleteTestUser() {
        ...
        //删除钱包
        userWalletDao.deleteAccount(user_id);
        //删除购买游戏
        playerGameDao.deleteAccount(user_id);
        //删除所有评论
        gameRatingDao.deleteAccount(user_id);
        //删除评论
        essayDao.deleteAccountMessage(user_id);
        //删除文章
        essayDao.deleteAccountEssay(user_id);
        //删除player表
        playerDao.deleteAccount(user_id);
        //删除user表
    ...
    //sss 删除测试用户的所有评分
    @Test
    public void deleteTestUserRating(){
        String username = "test";
        Integer user_id = userDao.getIdByUsername(username);
        //删除所有评论
        Integer count = gameRatingDao.deleteAccount(user_id);
        System.out.println("删除测试用户的评分 数量为: "+count);
    }
