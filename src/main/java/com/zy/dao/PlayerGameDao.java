...
public interface PlayerGameDao {
    ...
    public List<Integer> selectPlayerGameId(Long player_id);


    @Select("select is_have from player_game where player_id=#{player_id} and game_id=#{game_id}")
    public Integer selectGameIsBuy(Long player_id, Integer game_id);

    //增加游戏
    @Insert("insert into player_game values(#{player_id},#{game_id},#{create_time},#{status},#{is_have})")
    public Integer addPlayerGame(Long player_id,Integer game_id,Long create_time,Integer status,Integer is_have);
