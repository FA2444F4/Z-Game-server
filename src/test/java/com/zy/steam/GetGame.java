...
public class GetGame {
    ...
    public void getGame() {
        ...
        int appNum = 1;
        List<Integer> appidList=gameDao.selectGameIdWhichError();
        for (int appid : appidList) {
            ...
            try {
                ...
                String header_image = data.getString("header_image");
                Double price=0.0;
                if(data.has("price_overview")){

                    String moneyText = data.getJSONObject("price_overview").getString("final_formatted");
                    moneyText = moneyText.replaceAll("[¥\\s]", "");
                    moneyText = moneyText.replaceAll("[$\\s]", "");
                    price=Double.valueOf(moneyText);
                }

                JSONArray screenshots=data.getJSONArray("screenshots");
                int screenshot_length=screenshots.length();
                String screenshot1=null;
                String screenshot2=null;
                String screenshot3=null;
                for (int i = 0; i < screenshot_length; i++) {
                    String tempScreenshot=screenshots.getJSONObject(i).getString("path_full");
                    if(i==0){
                        screenshot1=tempScreenshot;
                    }else if(i==1){
                        screenshot2=tempScreenshot;
                    }else if(i==2){
                        screenshot3=tempScreenshot;
                    }
                }
                /*String dateText=data.getJSONObject("release_date").getString("date");
                dateText=dateText.trim();
                // 将日期字符串转换为Unix时间戳
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy年M月d日");
                TemporalAccessor accessor = formatter.parse(dateText);
                LocalDate date = LocalDate.of(accessor.get(ChronoField.YEAR), accessor.get(ChronoField.MONTH_OF_YEAR), accessor.get(ChronoField.DAY_OF_MONTH));
                long unixTimestamp = date.atStartOfDay().toInstant(ZoneOffset.UTC).getEpochSecond();
                Long create_time=Long.valueOf(unixTimestamp);*/
                Long create_time=1682513040L;

                Game game = new Game(appid, null, price, name, short_description, header_image, screenshot1, screenshot2, screenshot3, 1, create_time);
